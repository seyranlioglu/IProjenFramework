$(document).ready(function () {
    Projen.Client.UI.getSelect2Combobox("txtDepartmentId", "/Department/GetDepartments");
    Projen.Client.UI.getJqGrid(
        tableId = "jqgrid",
        divId = "pjqgrid",
        url = "/UserPosition/UserPositionListByPositionId?Id=" + $("#txtPositionId").val(),
        colNames = ['Id', 'Pozisyon Adı', 'Kullanıcı Adı', 'PositionId', 'Kullanıcı', "<button class='btn btn-xs btn-success' id='add' style='margin-right:5px' onclick=\"Projen.Client.UI.getJqGridNewRow('jqgrid');\"><i class='fa fa-plus'></i> Yeni Kayıt</button>"],
        colModel = [
            { key: true, hidden: true, name: 'Id', index: 'Id', editable: true },
            { name: 'PositionName', index: 'PositionName', editable: true, hidden: true },
            {
                name: 'UserId', index: 'UserName', editable: true, width: 80, edittype: 'select',
                edittype: "select", editoptions: {
                    dataInit: function (elem) {
                        ...
                        setTimeout(function () {
                            if (jqselected != null) {
                                if (jqselected.UserName != null)
                                    $('#txtUserId').append('<option value="' + jqselected.UserName + '">' + jqselected.UserId + '</option>');
                                Projen.Client.UI.getSelect2Combobox(elem.id, "/User/GetAllUsers?Id=" + (jqselected.UserName || 0));
                            } else {
                                Projen.Client.UI.getSelect2Combobox(elem.id, "/User/GetAllUsers?Id=0");
                            }
            ...
            {
                name: 'PositionId', index: 'PositionId', hidden: true, editable: true, editoptions: {
                    defaultValue: $("#txtPositionId").val()
                }
            },
            { name: 'UserName', index: 'UserId', hidden: true, editable: false },
            { name: 'act', index: 'act', sortable: false }
        ],
        editurl = "/UserPosition/AddOrUpdateUserPositionAjax",
        caption = "",
        multiselect = false,
        deleteurl = "/UserPosition/DeleteUserPosition",
    );
    ...
    $("a[href='#s2']").on('shown.bs.tab', function (e) {
        Projen.Client.UI.getJqGrid(
            tableId = "userright",
            divId = "pjquserright",
            url = "/FormUserRightSet/FormUserRightSetByPosition?Id=" + $("#txtPositionId").val(),
            ...
            editurl = "/FormUserRightSet/AddOrUpdateFormUserRightSet?UserTypes=2",
            caption = "",
            multiselect = false,
